# Project Memory

## Core
VS é COMERCIAL-CÊNTRICA (VS Core OS). Esteira: VS Tools / Departamentos / VS 360 / VS Custom.
Métricas de ouro: <1min resposta, 98,9% assertividade, R$ 50k MRR meta. Caso âncora: Taubaté.
Dark default, Deep Space Blue #050814 + Cyber Orange #FF5300. Poppins Black Italic + Montserrat + JetBrains Mono.
Desktop-only (1280px+). Supabase + Evolution API. Claude Haiku 4.5 (cliente) / Sonnet (interno).
Victor (técnico) + Danilo (comercial). Nichos dinâmicos via consultoria_nichos + useNichos().
Sem emoji, bullets em legenda ou jargão ("jornada", "mindset", "alavancar", "destravar").
Em /configuracoes: Scripts e Cadência são por TIER (Tools/Departamentos/360/Custom), NÃO por nicho de cliente. Nichos = segmento (Estética/Odonto/Advoc/VS AUTO).

## Memories
- [VS Core OS](mem://negocio/vs-core-os) — Norte estratégico 2026, métricas de ouro e proibições de discurso
- [Esteira de Tiers](mem://produto/esteira-tiers) — VS Tools / Departamentos / VS 360 / VS Custom com faixas de preço
- [Módulos Core OS](mem://produto/modulos-core-os) — 6 módulos (Comercial, Gatekeeper, Onboarding, Telemetria, Financeiro, Governança) e maturidade
- [Stack Core OS](mem://tecnologia/stack-core-os) — Stack oficial: React/Tailwind dark, Claude Haiku/Sonnet, JetBrains Mono para dados
- [Identidade Visual](mem://style/identidade-visual) — Brand Bible VS dark-mode com Barlow Condensed/Barlow, VS Blue #2E6FCC e Blue Light #4A8DE0
- [Rebranding 2026](mem://style/rebranding-2026) — Brutalismo Tech: Cyber Orange #FF5300 + Deep Space Blue #050814, Poppins Black Italic + Montserrat (substitui Barlow no app interno; toggle light disponível)
- [Fluxo Imersão](mem://funcionalidades/fluxo-imersao-regras) — Formulário 22 campos, mínimo 17 para conclusão
- [Automação Vendas](mem://funcionalidades/automacao-de-vendas-e-scripts) — Scripts A/B/C, cadência, follow-ups por nicho
- [Scripts por Tier](mem://funcionalidades/scripts-por-tier) — Configurações organiza scripts/cadência pelos 4 tiers da esteira (não por nicho de cliente)
- [Agente IA](mem://funcionalidades/agente-ia-monitoramento) — Dashboard de performance e monitoramento de automações
- [Pipeline Comercial](mem://funcionalidades/pipeline-comercial) — Kanban 11 estágios com filtros avançados
- [WhatsApp](mem://tecnologia/gestao-de-instancias-whatsapp) — Evolution API internalizada em Configurações
- [Meu Vendedor RAG](mem://funcionalidades/meu-vendedor-rag-adaptativo) — Base de Conhecimento com takeaways
- [Meu Vendedor IA](mem://funcionalidades/meu-vendedor-ia-vendas) — Simulador de treinamento de vendas
- [Prospecção](mem://funcionalidades/prospeccao-avancada-e-leads) — 6 etapas com Firecrawl e extração WhatsApp
- [Auth](mem://auth/requisitos-de-acesso) — Supabase Auth + vs_users obrigatório
- [Stack](mem://tecnologia/integracoes-e-stack) — React, TS, Tailwind, Supabase, Evolution API
- [Resiliência IA](mem://tecnologia/resiliencia-de-dados-ia) — Regex fallback para JSON de IA
- [Perfil Empresa](mem://negocio/perfil-da-empresa) — VS Growth Hub, consultoria marketing/vendas PMEs
- [Módulos Portados](mem://tecnologia/adaptacao-de-modulos-portados) — Adaptação para módulos independentes
- [Metodologia Comercial](mem://negocio/metodologia-comercial) — Dores por nicho, segmentação comercial
- [Devolutiva](mem://funcionalidades/devolutiva-e-documentacao) — Checklist de entrega, geração DOCX
- [Dashboard KPIs](mem://funcionalidades/dashboard-e-kpis) — 6 KPIs críticos
- [Desktop Only](mem://constraints/plataforma-alvo) — Sem mobile
- [Nichos Alvo](mem://negocio/nichos-alvo) — Scripts e config por nicho
- [Pós-Venda](mem://funcionalidades/automacao-pos-venda) — Acompanhamentos automáticos pós-fechamento
- [Diagnóstico](mem://funcionalidades/diagnostico-e-scoring) — 32 perguntas, 4 dimensões, scoring automático
- [Fluxo Operacional](mem://processo/fluxo-operacional) — Victor técnico, Danilo comercial, visibilidade total
- [ICP VS AUTO](mem://negocio/icp-vs-auto) — ICP completo do produto VS AUTO para revendas de seminovos
